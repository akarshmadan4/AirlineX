...
-- Altering the table to change the data type of the PHONE column to VARCHAR
...
ALTER TABLE EMPLOYEE1 MODIFY COLUMN PHONE VARCHAR(20);
...
ALTER TABLE TICKET1 MODIFY COLUMN TICKET_NUMBER VARCHAR(30);
ALTER TABLE TICKET1 MODIFY COLUMN DATE_OF_BOOKING VARCHAR(20);
ALTER TABLE TICKET1 MODIFY COLUMN DATE_OF_TRAVEL VARCHAR(20);
ALTER TABLE TICKET1 MODIFY COLUMN DATE_OF_CANCELLATION VARCHAR(20);
...
ALTER TABLE TICKET2 MODIFY COLUMN DATE_OF_BOOKING VARCHAR(20);
...
ALTER TABLE TICKET3 MODIFY COLUMN DATE_OF_CANCELLATION VARCHAR(20);
...
-- 1
SELECT FLIGHT_CODE, SOURCE, DESTINATION
FROM FLIGHT
WHERE SOURCE = 'BOM' OR DESTINATION = 'BOM';



-- 2
SELECT e.FNAME, e.LNAME, p.PID, p.PASSPORTNO
FROM EMPLOYEE1 e
INNER JOIN SERVES s ON e.SSN = s.SSN
INNER JOIN PASSENGER1 p ON s.PID = p.PID AND s.PASSPORTNO = p.PASSPORTNO;



-- 3
SELECT f.FLIGHT_CODE, f.SOURCE, f.DESTINATION
FROM FLIGHT f
INNER JOIN AIRLINE a ON f.AIRLINEID = a.AIRLINEID
WHERE a.AL_NAME = 'Air India Limited';



-- 4
SELECT JOBTYPE, AVG(SALARY) AS AVERAGE_SALARY
FROM EMPLOYEE2
GROUP BY JOBTYPE;

-- 5
SELECT ap.AP_NAME, COUNT(DISTINCT s.PID) AS NUM_PASSENGERS
FROM AIRPORT ap
LEFT JOIN EMPLOYEE1 e ON ap.AP_NAME = e.AP_NAME
LEFT JOIN SERVES s ON e.SSN = s.SSN
GROUP BY ap.AP_NAME
ORDER BY NUM_PASSENGERS DESC
LIMIT 1;

-- 6
SELECT p.PID, p.PASSPORTNO, COUNT(*) AS NUM_BOOKINGS
FROM PASSENGER1 p
INNER JOIN TICKET1 t ON p.PID = t.PID AND p.PASSPORTNO = t.PASSPORTNO
GROUP BY p.PID, p.PASSPORTNO
ORDER BY NUM_BOOKINGS DESC
LIMIT 5;

-- 7
SELECT FLIGHT_CODE, SOURCE, DESTINATION, DURATION
FROM FLIGHT
WHERE DURATION > '05:00:00';

-- 8
SELECT SOURCE, DESTINATION, COUNT(*) AS TicketCount
FROM TICKET1
GROUP BY SOURCE, DESTINATION;


-- 9
SELECT CLASS, COUNT(*) AS PassengersCount
FROM TICKET1
GROUP BY CLASS;

-- 10
SELECT AIRLINE.AL_NAME, COUNT(*) AS TotalTickets
FROM TICKET1
JOIN FLIGHT ON TICKET1.SOURCE = FLIGHT.SOURCE AND TICKET1.DESTINATION = FLIGHT.DESTINATION
JOIN AIRLINE ON FLIGHT.AIRLINEID = AIRLINE.AIRLINEID
GROUP BY AIRLINE.AL_NAME;
